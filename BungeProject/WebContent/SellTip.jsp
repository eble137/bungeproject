<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
	<meta charset="UTF-8">
	<title>번개장터 상품 판매 팁</title>
	<link rel="stylesheet" href="css/SellTip.css"/>
</head>
<body>
	<header>
		<div style="width: 50px; height: 50px;"></div>  
		<span>번개장터 상품 판매 팁</span>
		<div style="width: 50px; height: 50px; display: flex; justify-content: center; align-items: center; cursor: pointer;">
			<img onclick="window.close()" src="data:image/svg+xml;base64," alt="뒤로가기">
		</div>
	</header>
	<div id="guideCt1">
		<div id="guideCt2">
			<img class="img" style="padding-top: 50px;" src="Images/sellTip1.png">
			<img class="img" src="images/sellTip2.png">
			<img class="img" src="images/sellTip3.png">
			<img class="img" src="images/sellTip4.png">
		</div>
	</div>
	<footer>
		<button><strong>상품을 더 등록해볼까요?</strong></button>
	</footer>
</body>
</html>